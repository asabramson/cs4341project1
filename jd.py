...
# Heuristic evaluation (STILL IN PROGRESS DECIDING VALUES) material + mobility + control
def evaluate(state, player, is_root):
    ...
    score += 100 * (my_pieces_left - opp_pieces_left)
    ...
    score += 20 * (my_control - opp_control)
...
def iterative_deepening(state, player):
    ...
    while True:
        ...
        log_debug("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!VALUE OF MOVE IS: {}".format(value))
        if time.time() - start_time > TIME_LIMIT * 0.9:
            break
        if move is not None:
            best_move = move
